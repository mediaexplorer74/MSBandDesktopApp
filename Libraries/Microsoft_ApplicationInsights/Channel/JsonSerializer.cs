...
// Type: Microsoft.ApplicationInsights.Channel.JsonSerializer
...
namespace Microsoft.ApplicationInsights.Channel
{
  internal static class JsonSerializer
  {
    ...
    internal static string SerializeAsString(IEnumerable<ITelemetry> telemetryItems)
    {
      ...
      {
        JsonSerializer.SeializeToStream(telemetryItems, (TextWriter) stringWriter);
        return sb.ToString();
      }
    }
        ...
        if (exception.parsedStack.Count > 0)
        {
          ...
          foreach (StackFrame parsed in (IEnumerable<StackFrame>) exception.parsedStack)
          {
            if (num2++ != 0)
              writer.WriteComma();
            writer.WriteStartObject();
            JsonSerializer.SerializeStackFrame(parsed, writer);
    ...
    private static void SerializeTelemetryItem(ITelemetry telemetryItem, JsonWriter jsonWriter)
    {
      switch (telemetryItem)
      {
        ...
        case PerformanceCounterTelemetry _:
          ...
          break;
        default:
          string msg = string.Format((IFormatProvider) CultureInfo.InvariantCulture, "Unknown telemtry type: {0}", (object) telemetryItem.GetType());
      ...
      TextWriter streamWriter)
    {
      ...
      foreach (ITelemetry telemetryItem in telemetryItems)
      {
        ...
        JsonSerializer.SerializeTelemetryItem(telemetryItem, jsonWriter);
      }
    }
    ...
    {
      writer.WriteStartObject();
      eventTelemetry.WriteTelemetryName((IJsonWriter) writer, "Event");
