...
namespace Microsoft.Band.Admin
{
  public sealed class CargoCall : NotificationBase
  {
    ...
    {
      int num = 0 + (2 + ProtoFileTime.GetSerializedProtobufByteCount()) + (2 + ProtoGuid.GetSerializedProtobufByteCount()) + (1 + CodedOutputStream.ComputeInt32Size((int) this.Call_Id));
      if (!string.IsNullOrWhiteSpace(this.CallerName))
        num += 1 + CodedOutputStreamExtensions.ComputeStringSize(this.CallerName, 40);
      ...
      if (this.CallType != CargoCall.PhoneCallType.None)
        num += 1 + CodedOutputStream.ComputeEnumSize((int) this.CallType);
      if (this.CallType == CargoCall.PhoneCallType.Incoming)
        num = num + (1 + CodedOutputStream.ComputeUInt32Size(17U)) + (1 + CodedOutputStream.ComputeUInt32Size(24U));
      return num;
    }
