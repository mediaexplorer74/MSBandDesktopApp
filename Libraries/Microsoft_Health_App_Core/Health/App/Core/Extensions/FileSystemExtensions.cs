...
namespace Microsoft.Health.App.Core.Extensions
{
  public static class FileSystemExtensions
  {
      ...
      CancellationToken token = default (CancellationToken))
    {
      ...
      using (Stream inputStream = await file.OpenAsync(FileAccess.Read, token).ConfigureAwait(false))
      {
        using (Stream outputStream = await outputFile.OpenAsync(FileAccess.ReadAndWrite, token).ConfigureAwait(false))
    ...
    public static async Task WriteStreamAsync(
      ...
      CancellationToken token = default (CancellationToken))
    {
      using (Stream fileStream = await file.OpenAsync(FileAccess.ReadAndWrite, token).ConfigureAwait(false))
