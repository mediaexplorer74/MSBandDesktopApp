...
namespace Microsoft.Health.App.Core
{
  public static class CoreIoc
  {
    ...
    private static void RegisterDynamicConfigurationClient(UnityContainer container) => container.RegisterType<IDynamicConfigurationClient>((InjectionMember) new InjectionFactory((Func<IUnityContainer, object>) (c =>
    {
      ...
      return (object) new DynamicConfigurationClient(httpMessageHandler, (Func<CancellationToken, Task<Uri>>) (cancellationToken => (await connectionInfoProvider.GetConnectionInfoAsync(cancellationToken, true).ConfigureAwait(false)).FusEndpoint));
    })));
    ...
    private static void RegisterPodClient(UnityContainer container) => container.RegisterType<IHealthCloudClient>((InjectionMember) new InjectionFactory((Func<IUnityContainer, object>) (c =>
    {
      ...
      if (testService.IsEnabled)
        httpMessageHandler = httpMessageHandler.WithTestHeader(testService);
      IHealthCloudClient healthCloudClient = (IHealthCloudClient) new HealthCloudClient(httpMessageHandler, (Func<CancellationToken, Task<Uri>>) (cancellationToken => (await connectionInfoProvider.GetConnectionInfoAsync(cancellationToken, true).ConfigureAwait(false)).PodEndpoint), config.IsCachingEnabled ? c.Resolve<IHttpCacheService>() : (IHttpCacheService) null, (Func<CancellationToken, Task<Uri>>) (cancellationToken => (await connectionInfoProvider.GetConnectionInfoAsync(cancellationToken, true).ConfigureAwait(false)).SocialServiceEndPoint), (Func<CancellationToken, Task<string>>) (cancellationToken => (await connectionInfoProvider.GetConnectionInfoAsync(cancellationToken, true).ConfigureAwait(false)).PodSecurityToken));
    ...
    private static LifetimeManager Singleton => (LifetimeManager) new ContainerControlledLifetimeManager();
  }
}
