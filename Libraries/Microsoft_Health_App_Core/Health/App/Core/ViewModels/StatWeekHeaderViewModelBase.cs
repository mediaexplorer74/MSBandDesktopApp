...
// Type: Microsoft.Health.App.Core.ViewModels.StatWeekHeaderViewModelBase
...
{
  public abstract class StatWeekHeaderViewModelBase : IStatHeaderViewModel, ILoadableParameters
  {
    ...
    public StatWeekHeaderViewModelBase(
    ...
    {
      DateTimeOffset startDate = this.dateTimeService.GetTomorrowLastWeek();
      if (parameters != null && parameters.ContainsKey("StartDate"))
        startDate = DateTimeOffset.Parse(parameters["StartDate"]);
      this.SummaryGroup = (await this.userDailySummaryProvider.GetPreviousUserDailySummaryWeekGroupsAsync(startDate, 1)).FirstOrDefault<UserDailySummaryGroup>();
      this.StatHistoryEntry = new StatHistoryEntry(startDate, 0, true);
    }
  }
}
