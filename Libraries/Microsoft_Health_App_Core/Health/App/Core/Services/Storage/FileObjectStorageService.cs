...
namespace Microsoft.Health.App.Core.Services.Storage
{
  public sealed class FileObjectStorageService : IFileObjectStorageService
  {
      ...
      bool encrypt)
    {
      try
      {
        using (Stream fileStream = await (await (await this.fileSystemService.GetObjectStorageFolderAsync().ConfigureAwait(false)).CreateFileAsync(fileName, CreationCollisionOption.ReplaceExisting, token).ConfigureAwait(false)).OpenAsync(FileAccess.ReadAndWrite, token).ConfigureAwait(false))
        {
