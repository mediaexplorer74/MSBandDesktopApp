// Decompiled with JetBrains decompiler
// Type: Microsoft.Band.Admin.Desktop.WebTiles.ImageProvider
// Assembly: Microsoft.Band.Admin.Desktop, Version=1.3.20517.1, Culture=neutral, PublicKeyToken=null
// MVID: 14F573E4-478A-4BD1-B169-7232F63F8A40
// Assembly location: C:\Users\Pdawg\Downloads\Microsoft Band Sync Setup\Microsoft_Band_Admin_Desktop.dll

using Microsoft.Band.Admin.WebTiles;
using Microsoft.Band.Tiles;
using System;
using System.Threading.Tasks;
using System.Windows.Media.Imaging;

namespace Microsoft.Band.Admin.Desktop.WebTiles
{
  internal class ImageProvider : IImageProvider
  {
    private StorageProvider storageProvider;

    public ImageProvider(StorageProvider storageProvider) => this.storageProvider = storageProvider;

    public Task<BandIcon> GetBandIconFromFileAsync(string path) => Task.FromResult<BandIcon>(new WriteableBitmap((BitmapSource) new BitmapImage(new Uri(path))).ToBandIcon());
  }
}
