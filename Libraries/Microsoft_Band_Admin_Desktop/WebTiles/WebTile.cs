...
namespace Microsoft.Band.Admin.WebTiles
{
  ...
  public class WebTile : IWebTile
  {
    ...
    {
      get => this.version;
      set => this.validator.SetProperty<uint>(ref this.version, value, nameof (Version), 1U <= value, string.Format(CommonSR.WTVersionTooSmall, (object) 1U));
    }
    ...
    private void GenerateIconNameMappings()
    {
      ...
      foreach (KeyValuePair<string, string> additionalIcon in this.AdditionalIcons)
      {
        ...
        if (num < 0)
        {
          this.validator.CheckProperty("AdditionalIcons", this.iconFilenames.Count - 2 < 8, string.Format(CommonSR.WTMaxIconsExceeded, (object) 8));
    ...
    public WebTilePage[] Pages
    {
      ...
      set
      {
        this.validator.SetProperty<WebTilePage[]>(ref this.pages, value, nameof (Pages), value.Length <= 7, string.Format(CommonSR.WTTooManyPages, (object) 7));
      ...
      int pixelSize)
    {
      ...
      try
      {
        BandIcon iconFromFileAsync = await this.ImageProvider.GetBandIconFromFileAsync(Path.Combine(this.PackageFolderPath, iconFilename).Replace("/", "\\"));
        if (pixelSize > 0 && (iconFromFileAsync.Width != pixelSize || iconFromFileAsync.Height != pixelSize))
          throw new InvalidDataException(string.Format(CommonSR.WTInvalidIconDimensions, (object) iconName));
        bandIcon = iconFromFileAsync;
      }
      catch (BandException ex)
      {
        throw new InvalidDataException(string.Format(CommonSR.WTInvalidIconFile, (object) iconFilename), (Exception) ex);
      }
      return bandIcon;
    }
    ...
    {
      TileLayout[] tileLayoutArray = new TileLayout[1 + this.LayoutIndices.Count];
      tileLayoutArray[0] = new TileLayout(WebTilePage.GetLayoutBlob("MSBand_ScrollingText", bandClass));
      foreach (KeyValuePair<string, int> layoutIndex in this.LayoutIndices)
        tileLayoutArray[layoutIndex.Value] = new TileLayout(WebTilePage.GetLayoutBlob(layoutIndex.Key, bandClass));
      return tileLayoutArray;
    }));
      ...
      out NotificationDialog notificationDialog)
    {
      ...
      try
      {
        ...
        if (this.Resources[0].Style == ResourceStyle.Feed)
        {
          ...
          pageDataList = this.GetFeedPageDataList(out notificationDialog);
        }
        else
        {
      ...
      {
        Logger.LogException(Microsoft.Band.Admin.LogLevel.Error, ex, "Error occurred while refreshing Webtile, using error page instead.");
      ...
      Dictionary<string, string> variableMappings)
    {
      if (this.Notifications != null)
      {
        ...
        {
          Logger.Log(Microsoft.Band.Admin.LogLevel.Error, string.Format("Exception {0} processing WebTile notification", (object) ex));
        }
      }
    ...
    {
      PageData pageData = new PageData(new Guid("A5EECE73496945D1A863CFA76DC485FF"), 0, new PageElementData[0]);
      pageData.Values.Add((PageElementData) new TextBlockData((short) 1, "Data fetch error"));
      pageData.Values.Add((PageElementData) new TextBlockData((short) 2, "There seems to be something wrong with the data for this tile...check back in a few."));
      pageData.Values.Add((PageElementData) new TextBlockData((short) 3, string.Format("{0:MM/dd - hh:mm tt}", (object) DateTimeOffset.Now)));
      return pageData;
    }

    private List<PageData> GetFeedPageDataList(out NotificationDialog dialog)
    {
      if (this.Resources.Length != 1)
      ...
      List<PageData> pageDataList = new List<PageData>(result.Count);
    ...
    private List<PageData> GetSimplePageDataList(Dictionary<string, string> mappings)
    {
      ...
      for (int pageIndex = this.pages.Length - 1; pageIndex >= 0; --pageIndex)
        pageDataList.Add(this.GetPageData(pageIndex, mappings));
      return pageDataList;
    }
    ...
    private PageData GetPageData(int pageIndex, Dictionary<string, string> mappings)
    {
      ...
      if (pageIndex == -1)
      {
        ...
        pageId = new Guid(string.Format("{0}0{1}", (object) "A5EECE73496945D1A863CFA76DC485", (object) pageIndex));
    ...
    public bool HasRefreshIntervalElapsed(DateTimeOffset time)
    {
      ...
      WebTileSyncInfo webTileSyncInfo;
      using (Stream inputStream = this.StorageProvider.OpenFileForRead(StorageProviderRoot.App, Path.Combine(this.DataFolderPath, "LastSync.json"), 4096))
        webTileSyncInfo = CargoClient.DeserializeJson<WebTileSyncInfo>(inputStream);
      return webTileSyncInfo.LastSyncTime.AddMinutes((double) this.RefreshIntervalMinutes) < time;
    }

    internal void ReadResourceCache()
    {
      this.CheckStorageProviderAndDataFolderPath();
      string relativePath = Path.Combine(this.DataFolderPath, "ResourceCache.json");
      ...
      catch (Exception ex)
      {
        ...
        {
          Logger.Log(Microsoft.Band.Admin.LogLevel.Info, "Unexpected error opening resource cache file {0}", (object) ex);
          throw;
        }
      }
    ...
    internal void WriteResourceCache()
    {
      ...
      using (Stream outputStream = this.StorageProvider.OpenFileForWrite(StorageProviderRoot.App, Path.Combine(this.DataFolderPath, "ResourceCache.json"), false))
    ...
    {
      this.CheckStorageProviderAndDataFolderPath();
      WebTileSyncInfo webTileSyncInfo = new WebTileSyncInfo()
      {
        LastSyncTime = time
      };
      using (Stream outputStream = this.StorageProvider.OpenFileForWrite(StorageProviderRoot.App, Path.Combine(this.DataFolderPath, "LastSync.json"), false))
        CargoClient.SerializeJson(outputStream, (object) webTileSyncInfo);
    ...
    private string UserSettingsFilePath
    {
      ...
      {
        this.CheckStorageProviderAndDataFolderPath();
        return Path.Combine(this.DataFolderPath, "UserSettings.json");
      }
    }
    ...
    internal void ValidateTextBindings()
    {
      ...
      foreach (string str in referencedVariableNames1)
      {
        if (!definedVariableNames.Contains(str))
          throw new InvalidDataException(string.Format(CommonSR.WTUndefinedVariableReferencedInTextBindings, (object) str));
      }
      foreach (string str in referencedVariableNames2)
      {
        if (!definedVariableNames.Contains(str))
          throw new InvalidDataException(string.Format(CommonSR.WTUndefinedVariableReferencedInNotifications, (object) str));
      }
      foreach (object obj in definedVariableNames.Except<string>((IEnumerable<string>) referencedVariableNames1).Except<string>((IEnumerable<string>) referencedVariableNames2))
        Logger.Log(Microsoft.Band.Admin.LogLevel.Warning, string.Format("Resource variable {0} not used", obj));
    }
          ...
          if (privateResource != null && privateResource.Content != null)
          {
            foreach (KeyValuePair<string, string> keyValuePair in privateResource.Content)
            {
              ...
              if (stringSet.Contains(key))
                throw new InvalidDataException(string.Format(CommonSR.WTVariableNameNotUnique, (object) key));
              stringSet.Add(key);
            }
          }
        }
    ...
    internal void ValidateIconBindings()
    {
      ...
      foreach (string str in referencedIconNames)
      {
        if (!definedIconNames.Contains(str))
          throw new InvalidDataException(string.Format(CommonSR.WTIconNotDefined, (object) str));
      }
    }
    ...
    private HashSet<string> GetDefinedIconNames()
    {
      ...
      {
        foreach (KeyValuePair<string, string> additionalIcon in this.additionalIcons)
          stringSet.Add(additionalIcon.Key);
      }
      return stringSet;
    }
            ...
            foreach (WebTileIconBinding webTileIconBinding in iconBindings)
            {
              if (webTileIconBinding != null && webTileIconBinding.Conditions != null)
              {
                foreach (WebTileIconCondition condition in webTileIconBinding.Conditions)
                  stringSet.Add(condition.IconName);
              }
            }
          }
        }
    ...
    private static PageLayout DeserializeLayout(string layoutName)
    {
      ...
      if (layoutBlob != null)
      {
        using (MemoryStream memoryStream = new MemoryStream(layoutBlob))
        {
          using (CargoReaderOnBinaryReader readerOnBinaryReader = new CargoReaderOnBinaryReader(new BinaryReader((Stream) memoryStream)))
            pageLayout = PageLayout.DeserializeFromBand((ICargoReader) readerOnBinaryReader);
        }
      }
      return pageLayout;
    }
    ...
    private void ValidateTextBindingElementIds(WebTilePage page, PageLayout pageLayout)
    {
      ...
      foreach (WebTileTextBinding textBinding in page.TextBindings)
      {
        ...
        if (shortSet.Contains(textBinding.ElementId))
          page.Validator.CheckProperty("TextBindings", false, string.Format(CommonSR.WTMultipleTextBindingsWithElementId, (object) textBinding.ElementId));
        else
        ...
        {
          case null:
            textBinding.Validator.CheckProperty("ElementId", false, string.Format(CommonSR.WTElementIDNotValidForLayout, (object) textBinding.ElementId, (object) page.LayoutName));
            continue;
          case TextBlock _:
          ...
          case Barcode _:
            continue;
          default:
            textBinding.Validator.CheckProperty("ElementId", false, string.Format(CommonSR.WTElementIDDoesNotSupportText, (object) textBinding.ElementId, (object) element.ToString()));
            continue;
        }
      }
    }
    ...
    private void ValidateIconBindingElementIds(WebTilePage page, PageLayout pageLayout)
    {
      ...
      foreach (WebTileIconBinding iconBinding in page.IconBindings)
      {
        ...
        if (element == null)
          iconBinding.Validator.CheckProperty("ElementId", false, string.Format(CommonSR.WTElementIDNotValidForLayout, (object) iconBinding.ElementId, (object) page.LayoutName));
        else if (!(element is Icon))
          iconBinding.Validator.CheckProperty("ElementId", false, string.Format(CommonSR.WTElementIDIsNotAnIconInLayout, (object) iconBinding.ElementId, (object) page.LayoutName));
      }
    }
