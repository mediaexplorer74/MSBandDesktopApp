...
namespace Newtonsoft.Json.Utilities
{
  internal class ExpressionReflectionDelegateFactory : ReflectionDelegateFactory
  {
      ...
      ParameterExpression argsParameterExpression)
    {
      ...
      Expression expression4 = (object) (method as MethodInfo) == null ? this.EnsureCastExpression(expression3, type) : (!(((MethodInfo) method).ReturnType != typeof (void)) ? (Expression) Expression.Block(expression3, (Expression) Expression.Constant((object) null)) : this.EnsureCastExpression(expression3, type));
      if (byRefParameterList.Count > 0)
      {
        IList<ParameterExpression> parameterExpressionList = (IList<ParameterExpression>) new List<ParameterExpression>();
        ...
        foreach (ExpressionReflectionDelegateFactory.ByRefParameter byRefParameter in (IEnumerable<ExpressionReflectionDelegateFactory.ByRefParameter>) byRefParameterList)
        {
          ...
          parameterExpressionList.Add(byRefParameter.Variable);
        }
        expressionList.Add(expression4);
        expression4 = (Expression) Expression.Block((IEnumerable<ParameterExpression>) parameterExpressionList, (IEnumerable<Expression>) expressionList);
      }
      return expression4;
    }
