...
label_17:
      ...
      else
      {
        ...
        switch (ConvertUtils.Int64TryParse(this._stringReference.Chars, this._stringReference.StartIndex, this._stringReference.Length, out num))
        {
          ...
          case ParseResult.Overflow:
            ...
            obj = number.Length <= 380 ? JsonTextReader.BigIntegerParse(number, CultureInfo.InvariantCulture) : throw JsonReaderException.Create((JsonReader) this, "JSON integer {0} is too large to parse.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) this._stringReference.ToString()));
            newToken = JsonToken.Integer;
            break;
          default:
            string s = this._stringReference.ToString();
    ...
    public int LineNumber => this.CurrentState == JsonReader.State.Start && this.LinePosition == 0 ? 0 : this._lineNumber;
