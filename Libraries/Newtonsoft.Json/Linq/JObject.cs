    ...
    INotifyPropertyChanging
  {
    ...
    internal override void MergeItem(object content, JsonMergeSettings settings)
    {
      ...
      foreach (KeyValuePair<string, JToken> keyValuePair in jobject)
      {
        ...
        {
          if (!(jproperty.Value is JContainer jcontainer4))
          {
            if (keyValuePair.Value.Type != JTokenType.Null)
              jproperty.Value = keyValuePair.Value;
          }
          else if (jcontainer4.Type != keyValuePair.Value.Type)
            jproperty.Value = keyValuePair.Value;
          else
            jcontainer4.Merge((object) keyValuePair.Value, settings);
        }
      }
    }
    ...
    protected override DynamicMetaObject GetMetaObject(Expression parameter) => (DynamicMetaObject) new DynamicProxyMetaObject<JObject>(parameter, this, (DynamicProxy<JObject>) new JObject.JObjectDynamicProxy(), true);
