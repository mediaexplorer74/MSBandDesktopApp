...
namespace DesktopSyncApp
{
    ...
    IComponentConnector
  {
    ...
    internal AppMainWindow MainWindow1; // ME
    internal AnimatedPageControl MainWindowPageManager1; // ME
    internal AnimatedPageControl SuperModalPageManager1; // ME
    private bool _contentLoaded1; // ME
      ...
      using (new DisposableAction((Action) (() => this.model.AquiringDevice = true), (Action) (() => this.model.AquiringDevice = false)))
      {
        try
        {
          ...
          await this.model.DeviceManager.UpdateDeviceInfo(deviceInfo, this.model.LoginInfo, false, false);
                ...
                if (
                                (int)deviceInfo.AppType != 2
                                && (int)deviceInfo.AppType != 3
                                )
                {
                    this.model.UserDeviceStatus = UserDeviceStatus.None;
                }
                ...
                else if (pairDevice || (int)deviceInfo.DeviceProfileStatus.UserLinkStatus == 1 
...
label_7:
    ...
    public async Task PushFirmwareUpdate()
    {
      ...
      try
      {
        ...
        {
                    // RnD, TODO
                    int num = await this.model.DeviceManager.CurrentDevice.CargoClient.UpdateFirmwareAsync
                    (
                    info,
                    CancellationToken.None,
                    null//(IProgress<FirmwareUpdateProgress>)this.model.DeviceManager.CurrentDevice.FirmwareUpdateProgress
                    )
                    ? 1 : 0;                    
        }
      ...
      if 
      (
        success && deviceProfileStatus != null 
...
label_9:
    ...
    public async Task GetServiceInfo()
    {
      ...
      using (ICargoClient cargoClient = await BandAdminClientManager.Instance.ConnectAsync(loginInfo1.ServiceInfo))
      {
        ...
        ViewModel1 model = this.model; // ME
        
        loginInfo2.UserProfile = new UserProfileSurrogate(userProfileAsync, model);
        loginInfo2 = (LoginInfo) null;
        loginInfo1.UserProfileEdit = new UserProfileEdit(
            loginInfo1.UserProfile, false, 
    ...
    public AppMainWindow(DesktopSyncApp.App app, ViewModel1 model)
    {
      this.app = app;
      this.model = model;
    ...
    public ViewModel1 Model => this.model; // ME
    ...
    private async void AutoSyncChanged(object sender, PropertyValueChangedEventArgs args)
    {
      if ((bool) args.NewValue)
      {
        if 
        (
            this.model.DeviceManager.CurrentDevice == null 
            || this.model.LoginInfo == null 
            || this.model.DeviceManager.CurrentDevice.Syncing
        )
          return;
        ...
        private async void btSync_Click(object sender, RoutedEventArgs e)
        {
            await this.SyncDeviceToCloud();
        }
    ...
    [DebuggerNonUserCode]
    [GeneratedCode("PresentationBuildTasks", "4.0.0.0")]
    internal Delegate _CreateDelegate1(Type delegateType, string handler) 
            => Delegate.CreateDelegate(delegateType, (object) this, handler);
    */
