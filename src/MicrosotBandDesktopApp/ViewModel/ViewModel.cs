...
namespace DesktopSyncApp
{
  ...
  public class ViewModel1 : INotifyPropertyChanged
  {
    ...
    private DynamicGlobalizationConfiguration dynamicGlobalizationConfig;
        ...
        public ViewModel1(App app, DynamicSettings dynamicSettings, CommandLineSettings commandLineSettings)
        {
            App = app;
    ...
    public async Task UpdateLoginInfoWithMsaUserProfile()
    {
      try
      {
        ...
        if (string.IsNullOrWhiteSpace(this.LoginInfo.UserProfileEdit.FirstName) || !string.IsNullOrWhiteSpace(this.LoginInfo.UserProfileEdit.DeviceName))
          return;
        string str = string.Format(Strings.OobeBandNameString, (object) this.LoginInfo.UserProfileEdit.FirstName);
        if (str.Length > 15)
          return;
    ...
    public async Task RefreshUserProfile(bool force)
    {
      ...
      using (new DisposableAction((Action) (() => this.loginInfo.UpdatingUserProfile = true), (Action) (() => this.loginInfo.UpdatingUserProfile = false)))
      {
        ...
        {
          this.LogProfileUpdateError(new ErrorInfo("LoadUserProfile", Strings.Message_ProfileRefreshErrorOccurred, ex));
        }
      }
    }
    ...
    private async Task LoadUserProfile()
    {
      ...
      {
        this.LogProfileUpdateError(new ErrorInfo(nameof (LoadUserProfile), Strings.Message_ProfileRefreshErrorOccurred, ex));
        throw;
      }
    }
    ...
    private async Task SaveUserProfile()
    {
      ...
      catch (Exception ex)
      {
        ...
        this.LogProfileUpdateError(new ErrorInfo(nameof (SaveUserProfile), Strings.Message_ProfileSaveErrorOccurred, ex));
        return;
      }
      finally
      {
