...
// Type: Cirrious.CrossCore.Converters.MvxValueConverter`1
...
namespace Cirrious.CrossCore.Converters
{
  public abstract class MvxValueConverter<TFrom> : IMvxValueConverter
  {
      ...
      CultureInfo culture)
    {
      try
      {
        return (object) this.TypedConvertBack(value, targetType, parameter, culture);
      }
      catch (Exception ex)
      {
        return (object) MvxBindingConstant.UnsetValue;
    ...
    protected virtual TFrom TypedConvertBack(
      object value,
      Type targetType,
      object parameter,
      CultureInfo culture)
