      ...
      this object input)
    {
      ...
      if (input is IDictionary<string, string>)
        return (IDictionary<string, string>) input;
            ...
            //IEnumerable<_AnonymousType6<bool, PropertyInfo>> 
        IEnumerable datas = 
        input.GetType().GetProperties(Cirrious.CrossCore.BindingFlags.Instance 
        | Cirrious.CrossCore.BindingFlags.Public 
        | Cirrious.CrossCore.BindingFlags.FlattenHierarchy).Where<PropertyInfo>
        ((Func<PropertyInfo, bool>) (property => property.CanRead)).Select(
            property => new
      {
        CanSerialize = 
          MvxSingleton<IMvxSingletonCache>.Instance.Parser.TypeSupported(
