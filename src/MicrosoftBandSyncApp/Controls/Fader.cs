...
namespace DesktopSyncApp
{
  ...
  {
    public static readonly DependencyProperty FadedVisibilityProperty 
            = DependencyProperty.Register(nameof (FadedVisibility), 
                typeof (Visibility), typeof (Fader), 
                new PropertyMetadata((object) Visibility.Visible, 
                    new PropertyChangedCallback(Fader.FadedVisibilityPropertyChanged)));

    public static readonly DependencyProperty FadedOpacityProperty 
            = DependencyProperty.Register(nameof (FadedOpacity), 
                typeof (double), typeof (Fader), 
                new PropertyMetadata((object) 1.0, 
                    new PropertyChangedCallback(Fader.FadedOpacityPropertyChanged)));
    ...
    public Fader()
    {
      ...
      Storyboard.SetTargetProperty((DependencyObject) this.fadeStoryboard, 
          new PropertyPath("Opacity", new object[0]));
    ...
    private void SetFadedVisibility(Visibility value)
    {
      ...
      if (value == Visibility.Visible)
      {
        if (this.FadeinTime > 0U && (this.Visibility != Visibility.Visible || this.Opacity < 1.0))
        {
          ...
          if (this.Visibility == Visibility.Visible)
            this.fadeStoryboard.Seek(TimeSpan.FromMilliseconds(
                this.fadeStoryboard.Duration.TimeSpan.TotalMilliseconds 
      ...
      else if (this.FadeoutTime > 0U && this.Visibility == Visibility.Visible && this.Opacity > 0.0)
      {
        ...
        this.fadeStoryboard.Seek(TimeSpan.FromMilliseconds(
            this.fadeStoryboard.Duration.TimeSpan.TotalMilliseconds 
            * (Math.Max(this.FadedOpacity - this.Opacity, 0.0) / this.FadedOpacity)));
      }
      else
        this.Visibility = value;
    }
