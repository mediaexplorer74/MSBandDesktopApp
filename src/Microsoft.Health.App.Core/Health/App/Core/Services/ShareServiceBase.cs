...
namespace Microsoft.Health.App.Core.Services
{
  public abstract class ShareServiceBase : IShareService
  {
      ...
      CancellationToken cancellationToken)
    {
      ...
      Stream stream = await this.Request.ShareImageThumbnail.OpenAsync(PCLStorage.FileAccess.Read);
      ...
      CancellationToken cancellationToken)
    {
      ...
      using (HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, this.Request.ShareUrls.ShareImageUrl))
      {
        ...
        {
          using (Stream outputStream = await imageFile.OpenAsync(PCLStorage.FileAccess.ReadAndWrite, cancellationToken))
          {
