...
namespace Microsoft.Health.App.Core.Services
{
  public class TileUpdateService : ITileUpdateService
  {
    private static readonly ILog Logger = 
            LogManager.GetFileLogger("C:\\Builds\\6968\\6170\\Sources\\Source\\Windows\\KApp\\Core\\Services\\TileUpdateService.cs");
    ...
    )
    {
      ...
      Func<CancellationToken, Task>[] funcArray = new Func<CancellationToken, Task>[4]
      {
        (Func<CancellationToken, Task>) (async token =>
        {
          ...
          TileUpdateService._DisplayClass18_0 cDisplayClass180;
          
          // ISSUE: reference to a compiler-generated field
          long weatherElapsed1 = cDisplayClass180.weatherElapsed;
          long num = await this.DoUpdateTileAsync((IList<Exception>) exceptions, 
              "Weather", this.UpdateWeatherAsync(token), timedTelemetryEvent);

          // ISSUE: reference to a compiler-generated field
          cDisplayClass180.weatherElapsed = num;
          cDisplayClass180 = (TileUpdateService._DisplayClass18_0) null;
        }),

            */
        //public 
        Task SyncWebTilesAsync(bool forceSync, CancellationToken cancellationToken1)
        {
            throw new NotImplementedException();
        }

        //public 
        Task SyncWebTileAsync(Guid tileId, CancellationToken cancellationToken1)
        {
            throw new NotImplementedException();
        }
        ...
        (Func<CancellationToken, Task>) (async token =>
        {
          ...
          long num = await this.DoUpdateTileAsync((IList<Exception>) exceptions, 
              "Finance", this.UpdateFinanceAsync(token), timedTelemetryEvent);
          // ISSUE: reference to a compiler-generated field
        ...
        (Func<CancellationToken, Task>) (async token =>
        {
          ...
          long num = await this.DoUpdateTileAsync((IList<Exception>) exceptions, 
              "Calendar", this.UpdateCalendarAsync(token), timedTelemetryEvent);
    ...
    //public 
    async Task SyncWebTilesAsync(bool forceSync, CancellationToken cancellationToken)
    {
    ...
    //public 
    async Task SyncWebTileAsync(Guid tileId, CancellationToken cancellationToken)
    {
      using (IBandConnection cargoConnection = await this.cargoConnectionFactory.CreateConnectionAsync(cancellationToken))
        await cargoConnection.SyncWebTileAsync(tileId, cancellationToken);
    }

        Task ITileUpdateService.SyncWebTilesAsync(bool forceSync, CancellationToken cancellationToken)
        {
            throw new NotImplementedException();
        }

        Task ITileUpdateService.SyncWebTileAsync(Guid tileId, CancellationToken cancellationToken)
        {
            throw new NotImplementedException();
        }
    }
