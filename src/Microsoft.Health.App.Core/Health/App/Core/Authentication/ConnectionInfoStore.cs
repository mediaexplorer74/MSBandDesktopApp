  ...
  public class ConnectionInfoStore : IConnectionInfoStore
  {
    ...
    public async Task SetAsync(HealthCloudConnectionInfo connectionInfo)
    {
      try
      {
        ...
        using (Stream stream = await (await (await this.GetDataFolderAsync().ConfigureAwait(false)).CreateFileAsync
                    (this.DataFileName, 
                    CreationCollisionOption.ReplaceExisting)
                    .ConfigureAwait(false)).OpenAsync(PCLStorage.FileAccess.ReadAndWrite).ConfigureAwait(false))
