  ...
  internal sealed class CargoClient : BandClient, ICargoClient, IBandClient, IDisposable
  {
      ...
      SyncTasks syncTasks)
    {
      ...
      try
      {
        ...
        {
          InvalidOperationException operationException = new InvalidOperationException(CommonSR.OperationRequiredCloudConnection);
          Logger.LogException(LogLevel.Error, (Exception) operationException);
    ...
    public long GetPendingLocalDataBytes()
    {
      ...
      foreach (string file in this.storageProvider.GetFiles("PendingData"))
      {
        ...
        {
          string relativePath = Path.Combine("PendingData", file);
          num += this.storageProvider.GetFileSize(relativePath);
        }
      }
      return num;
    }
    ...
    public IUserProfile GetUserProfileFromDevice()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoProfileGetDataApp, 4, byteCount, writeArgBuf, CommandStatusHandling.DoNotCheck))
      {
        UserProfile userProfile = UserProfile.DeserializeAppDataFromBand((ICargoReader) cargoCommandReader, this.ConnectedAdminBandConstants);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        userProfile.DeviceSettings.DeviceId = this.DeviceUniqueId;
        userProfile.DeviceSettings.SerialNumber = this.SerialNumber;
    ...
    public IUserProfile GetUserProfile(CancellationToken cancellationToken)
    {
      ...
      if (this.cloudProvider == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException("cloudProvider");
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
      ...
      Guid cloudDeviceId)
    {
      ...
      deviceProfileStatus.UserLinkStatus = !(cloudDeviceId == Guid.Empty) ? (!(cloudDeviceId == this.DeviceUniqueId) ? LinkStatus.NonMatching : LinkStatus.Matching) : LinkStatus.Empty;
      deviceProfileStatus.DeviceLinkStatus = !(userProfileHeader.UserID == Guid.Empty) ? (!(userProfileHeader.UserID == cloudUserId) ? LinkStatus.NonMatching : LinkStatus.Matching) : LinkStatus.Empty;
      Logger.Log(LogLevel.Info, "Checking DeviceProfileLink: (UserDeviceID: {2} == DeviceID: {3}) is UserLinkStatus: {0} && (DeviceProfileID: {4} == UserProfileID: {5}) is DeviceLinkStatus: {1}", (object) deviceProfileStatus.UserLinkStatus, (object) deviceProfileStatus.DeviceLinkStatus, (object) cloudDeviceId, (object) this.DeviceUniqueId, (object) userProfileHeader.UserID, (object) cloudUserId);
    ...
    public void SyncUserProfile(CancellationToken cancellationToken)
    {
      ...
      UserProfileHeader userProfileHeader = this.ProfileAppHeaderGet();
      this.GetDeviceMasteredUserProfileProperties(userProfile, false);
      DateTimeOffset? lastKdkSyncUpdateOn = userProfile.LastKDKSyncUpdateOn;
...
label_9:
      ...
      if (flag1)
      {
        byte[] firmwareBytes = this.ProfileGetFirmwareBytes();
        userProfile.DeviceSettings.FirmwareByteArray = firmwareBytes;
        this.SaveUserProfile((IUserProfile) userProfile, cancellationToken, userProfile.LastKDKSyncUpdateOn);
      }
      else
        this.SaveUserProfileFirmwareBytes(cancellationToken);
    }
    ...
    private void GetDeviceMasteredUserProfileProperties(UserProfile profile, bool forExplicitSave)
    {
      int byteCount = UserProfile.GetAppDataSerializedByteCount(this.ConnectedAdminBandConstants);
      Action<ICargoWriter> writeArgBuf = (Action<ICargoWriter>) (w => w.WriteInt32(byteCount));
      Logger.Log(LogLevel.Info, "Obtaining the device mastered profile settings from the band");
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoProfileGetDataApp, 4, byteCount, writeArgBuf, CommandStatusHandling.ThrowOnlySeverityError))
        profile.DeserializeAndOverwriteDeviceMasteredAppDataFromBand((ICargoReader) cargoCommandReader, this.ConnectedAdminBandConstants, forExplicitSave);
    }
    ...
    private UserProfile GetDeviceMasteredUserProfileProperties()
    {
      ...
      Logger.Log(LogLevel.Info, "Obtaining the device mastered profile settings from the band");
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoProfileGetDataApp, 4, byteCount, writeArgBuf, CommandStatusHandling.ThrowOnlySeverityError))
        return UserProfile.DeserializeDeviceMasteredAppDataFromBand((ICargoReader) cargoCommandReader, this.ConnectedAdminBandConstants);
    }
    ...
    public DateTime GetDeviceUtcTime()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoTimeGetUtcTime, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        return CargoFileTime.DeserializeFromBandAsDateTime((ICargoReader) cargoCommandReader);
    }
    ...
    public void SetDeviceUtcTime(DateTime utc)
    {
      ...
      int serializedByteCount = CargoFileTime.GetSerializedByteCount();
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoTimeSetUtcTime, serializedByteCount, CommandStatusHandling.DoNotThrow))
        CargoFileTime.SerializeToBandFromDateTime((ICargoWriter) cargoCommandWriter, utc);
    }
    ...
    public CargoTimeZoneInfo GetDeviceTimeZone()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoSystemSettingsGetTimeZone, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        CargoTimeZoneInfo cargoTimeZoneInfo = CargoTimeZoneInfo.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
    ...
    public void SetDeviceTimeZone(CargoTimeZoneInfo timeZone)
    {
      ...
      this.CheckIfDisconnectedOrUpdateMode();
      int serializedByteCount = CargoTimeZoneInfo.GetSerializedByteCount();
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoSystemSettingsSetTimeZone, serializedByteCount, CommandStatusHandling.DoNotCheck))
        timeZone.SerializeToBand((ICargoWriter) cargoCommandWriter);
    }
    ...
    private bool GetFirmwareBinariesValidationStatusInternal()
    {
      ...
      if (this.DeviceTransport == null)
      {
        InvalidOperationException operationException = new InvalidOperationException(CommonSR.OperationRequiredConnectedDevice);
        Logger.LogException(LogLevel.Error, (Exception) operationException);
        throw operationException;
      }
      ...
      {
        this.DeviceTransport.CargoStream.ReadTimeout = 10000;
        cargoCommandReader.ReadExactAndDiscard(count);
        flag = cargoCommandReader.ReadBool32();
      }
      Logger.Log(LogLevel.Info, "Firmware binaries validation status: {0}", flag ? (object) "Valid" : (object) "Invalid");
      return flag;
    }
    ...
    public EphemerisCoverageDates GetGpsEphemerisCoverageDatesFromDevice()
    {
      ...
      try
      {
        ...
        using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoGpsEphemerisCoverageDates, bytesToRead, CommandStatusHandling.DoNotCheck))
        {
          ephemerisCoverageDates.StartDate = new DateTime?(CargoFileTime.DeserializeFromBandAsDateTime((ICargoReader) cargoCommandReader));
          ephemerisCoverageDates.EndDate = new DateTime?(CargoFileTime.DeserializeFromBandAsDateTime((ICargoReader) cargoCommandReader));
          if ((int) cargoCommandReader.CommandStatus.Status != (int) DeviceStatusCodes.Success)
          {
            ephemerisCoverageDates.StartDate = new DateTime?();
            ephemerisCoverageDates.EndDate = new DateTime?();
      ...
      bool forceUpdate)
    {
      ...
      cancellationToken.ThrowIfCancellationRequested();
      string relativePath = Path.Combine("Ephemeris", "EphemerisUpdate.bin");
    ...
    private void VerifyLocalEphemerisFile(
      ...
      bool forceUpdate)
    {
      string path2_1 = string.Format("{0}.temp", (object) "EphemerisVersion.json");
      string path2_2 = string.Format("{0}.temp", (object) "EphemerisUpdate.bin");
      string relativePath1 = Path.Combine("Ephemeris", "EphemerisVersion.json");
      string relativePath2 = Path.Combine("Ephemeris", "EphemerisUpdate.bin");
      string str1 = Path.Combine("Ephemeris", path2_1);
      string str2 = Path.Combine("Ephemeris", path2_2);
      ...
      if (!forceUpdate)
      {
        if (this.storageProvider.FileExists(relativePath1))
        {
          ...
          flag = this.storageProvider.FileExists(Path.Combine("Ephemeris", "EphemerisUpdate.bin"));
      ...
      try
      {
        using (Stream outputStream = this.storageProvider.OpenFileForWrite(str1, false, 1024))
      ...
      try
      {
        using (Stream updateStream = this.storageProvider.OpenFileForWrite(str2, false))
        {
      ...
      try
      {
        this.storageProvider.RenameFile(str2, "Ephemeris", "EphemerisUpdate.bin");
        this.storageProvider.RenameFile(str1, "Ephemeris", "EphemerisVersion.json");
      }
      catch (Exception ex)
      {
        throw new BandException(CommonSR.EphemerisDownloadError, ex);
      ...
      IUserProfile profile)
    {
      ...
      string relativePath1 = Path.Combine("TimeZoneData", "TimeZoneData.json");
      ...
      {
        BandCloudException bandCloudException = new BandCloudException(CommonSR.TimeZoneDataVersionDownloadError);
        Logger.LogException(LogLevel.Error, (Exception) bandCloudException);
        throw bandCloudException;
      }
      ...
      stopwatch.Reset();
      stopwatch.Start();
      string relativePath2 = Path.Combine("TimeZoneData", "TimeZoneUpdate.bin");
      string str = Path.Combine("TimeZoneData", "TimeZoneUpdateTemp.bin");
      ...
      {
        BandException bandException = new BandException(string.Format(CommonSR.TimeZoneDownloadTempFileOpenError, (object) str), ex);
      ...
      if (!flag || fileSize == 0)
      {
        BandCloudException bandCloudException = new BandCloudException(CommonSR.TimeZoneDataDownloadError);
        Logger.LogException(LogLevel.Error, (Exception) bandCloudException);
        throw bandCloudException;
      }
...
label_56:
      ...
      bool checkVersionFileBeforeDownload)
    {
      ...
      {
        ArgumentException argumentException = new ArgumentException(CommonSR.UnsupportedFileTypeToObtainFromDevice);
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
      ...
      if (this.storageProvider.DirectoryExists(str) && this.storageProvider.GetFiles(str).Length >= 20)
        flag = false;
      if (flag)
        flag = this.GetFileFromDevice(fileIndex, checkVersionFileBeforeDownload);
      progress.AddStepsCompleted(1);
      ...
      if (localDeviceFileSyncTimeInfo != null)
      {
        ...
        if (fileIndex == FileIndex.Instrumentation)
        {
          ...
          if (downloadAttemptTime.HasValue)
          {
            ...
            bool flag = timeSpan2 >= timeSpan1;
            Logger.Log(LogLevel.Info, "Device file check {0}required; File Index: {1}, Minimum Time Between Checks: {2}, Last Checked: {3:MM/dd/yyyy HH:mm} ({4})", flag ? (object) "" : (object) "not ", (object) fileIndex, (object) timeSpan1, (object) downloadAttemptTime.Value.ToLocalTime(), (object) timeSpan2);
            return flag;
          }
        }
        else
        {
          ArgumentException argumentException = new ArgumentException(CommonSR.UnsupportedFileTypeToObtainFromDevice);
          Logger.LogException(LogLevel.Warning, (Exception) argumentException);
          throw argumentException;
        }
      }
    ...
    {
      string str1 = fileIndex.ToString();
      ...
      switch (fileIndex)
      {
        ...
        case FileIndex.CrashDump:
          ...
          break;
        default:
          ArgumentException argumentException = (ArgumentException) new ArgumentOutOfRangeException(nameof (fileIndex), CommonSR.UnsupportedFileTypeToObtainFromDevice);
          Logger.LogException(LogLevel.Error, (Exception) argumentException);
          throw argumentException;
      }
      string str2 = Path.Combine(str1, "DeviceFileSyncTimeInfo.json");
      DeviceFileSyncTimeInfo localDeviceFileSyncTimeInfo = (DeviceFileSyncTimeInfo) null;
      if (checkVersionFileBeforeDownload)
      {
        localDeviceFileSyncTimeInfo = this.GetLocalDeviceFileSyncTimeInfo(str2);
      ...
      else
      {
        ...
        this.storageProvider.CreateFolder(str1);
        string format = string.Format("{{0}}-{{1:{0}}}.bin", (object) "yyyyMMddHHmmssfff");
        using (Stream stream = this.storageProvider.OpenFileForWrite(Path.Combine(str1, string.Format(format, (object) fileIndex, (object) DateTime.UtcNow)), false))
        {
          try
          {
          ...
          {
            Logger.LogException(LogLevel.Error, (Exception) new BandException(string.Format(CommonSR.DeviceFileDownloadError, (object) fileIndex), ex));
            throw;
          }
        }
      ...
      if (fileIndex == FileIndex.Instrumentation)
        this.SaveDeviceFileSyncTimeInfoLocally(str2, localDeviceFileSyncTimeInfo, fileIndex);
      return size > 0;
    }
      ...
      List<KeyValuePair<string, string>> queryParams = null)
    {
      ...
      {
        BandCloudException bandCloudException = new BandCloudException(CommonSR.FirmwareUpdateInfoError);
        Logger.LogException(LogLevel.Error, (Exception) bandCloudException);
        throw bandCloudException;
      }
      if (availableFirmwareVersion.IsFirmwareUpdateAvailable)
      {
        ...
        {
          BandException bandException = new BandException(CommonSR.InvalidUpdateDataSize);
          Logger.LogException(LogLevel.Error, (Exception) bandException);
          throw bandException;
        }
      }
    ...
    {
      using (StreamReader streamReader = new StreamReader(inputStream, Encoding.UTF8, false, 128, true))
        return CargoClient.DeserializeJson<T>((TextReader) streamReader);
    }

    internal static T DeserializeJson<T>(string input)
    {
      using (StringReader stringReader = new StringReader(input))
        return CargoClient.DeserializeJson<T>((TextReader) stringReader);
    }
      ...
      CancellationToken cancellationToken)
    {
      ...
      List<LogProcessingStatus> processingStatusList1 = new List<LogProcessingStatus>();
      List<LogProcessingStatus> processingStatusList2 = new List<LogProcessingStatus>();
      int num1 = 0;
      bool flag = true;
      ...
      while (source.Count > 0)
      {
          ...
          else
          {
            switch (uploadStatusInfo.UploadStatus)
            {
              ...
              case LogUploadStatus.EventsProcessingDone:
                processingStatusList1.Add(processingStatus);
                flag = true;
                dictionary.Remove(processingStatus.UploadId);
                ++num2;
                continue;
              default:
                processingStatusList2.Add(processingStatus);
                flag = true;
                ++num4;
                continue;
            }
        ...
        {
          this.DoLogCallback(notificationHandler, (IEnumerable<LogProcessingStatus>) processingStatusList1, (IEnumerable<LogProcessingStatus>) dictionary.Values, (IEnumerable<LogProcessingStatus>) processingStatusList2);
          flag = false;
        }
      }
    ...
    public void SetGoals(Goals goals)
    {
      ...
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoGoalTrackerSet, Goals.GetSerializedByteCount(this.ConnectedAdminBandConstants), CommandStatusHandling.DoNotCheck))
      {
        goals.SerializeToBand((ICargoWriter) cargoCommandWriter, this.ConnectedAdminBandConstants);
    ...
    public void SetWorkoutPlan(Stream workoutPlansStream)
    {
      ...
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (workoutPlansStream));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
    ...
    public void SetWorkoutPlan(byte[] workoutPlanData)
    {
      ...
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (workoutPlanData));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      using (MemoryStream memoryStream = new MemoryStream(workoutPlanData, false))
        this.UploadFitnessPlan((Stream) memoryStream, workoutPlanData.Length);
    }
    ...
    internal IList<WorkoutActivity> GetWorkoutActivities()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedAppDataGetWorkoutActivities, bytesToRead, CommandStatusHandling.ThrowOnlySeverityError))
      {
        List<WorkoutActivity> source = new List<WorkoutActivity>(15);
        for (int index = 0; index < 15; ++index)
          source.Add(WorkoutActivity.DeserializeFromBand((ICargoReader) cargoCommandReader));
        uint val1 = cargoCommandReader.ReadUInt32();
    ...
    internal void SetWorkoutActivities(IList<WorkoutActivity> activities)
    {
      ...
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoPersistedAppDataSetWorkoutActivities, dataSize, CommandStatusHandling.ThrowOnlySeverityError))
      {
        int index;
        for (index = 0; index < activities.Count; ++index)
          activities[index].SerializeToBand((ICargoWriter) cargoCommandWriter);
        for (; index < 15; ++index)
          cargoCommandWriter.WriteByte((byte) 0, serializedByteCount);
        cargoCommandWriter.WriteUInt32((uint) activities.Count);
        cargoCommandWriter.WriteByte((byte) 0, 10);
      }
    }
    ...
    public void SetGolfCourse(byte[] golfCourseData)
    {
      ...
      Logger.Log(LogLevel.Info, "Setting golf course data on the device");
      using (MemoryStream memoryStream = new MemoryStream(golfCourseData, false))
        this.UploadGolfCourse((Stream) memoryStream, golfCourseData.Length);
    }
    ...
    public string[] GetPhoneCallResponses()
    {
      ...
      Array.Copy((Array) allResponses, 0, (Array) strArray2, 0, 4);
      return strArray1;
    }
      ...
      string response4)
    {
      ...
      if (response1 == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (response1));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (response2 == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (response2));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (response3 == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (response3));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (response4 == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (response4));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (response1.Length > 160)
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response1), (object) 160));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (response2.Length > 160)
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response2), (object) 160));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (response3.Length > 160)
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response3), (object) 160));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (response4.Length > 160)
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response4), (object) 160));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
    ...
    public string[] GetSmsResponses()
    {
      ...
      string[] strArray1 = new string[4];
      string[] strArray2 = strArray1;
      Array.Copy((Array) allResponses, 4, (Array) strArray2, 0, 4);
      return strArray1;
    }
      ...
      string response4)
    {
      ...
      if (response1.Length > 160)
      {
        Logger.Log(LogLevel.Warning, string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response1), (object) 160));
        response1 = response1.Substring(0, 160);
      }
      if (response2.Length > 160)
      {
        Logger.Log(LogLevel.Warning, string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response2), (object) 160));
        response2 = response2.Substring(0, 160);
      }
      if (response3.Length > 160)
      {
        Logger.Log(LogLevel.Warning, string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response3), (object) 160));
        response3 = response3.Substring(0, 160);
      }
      if (response4.Length > 160)
      {
        Logger.Log(LogLevel.Warning, string.Format(CommonSR.ResponseStringExceedsMaxLength, (object) nameof (response4), (object) 160));
    ...
    public CargoRunDisplayMetrics GetRunDisplayMetrics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedAppDataGetRunMetrics, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        return CargoRunDisplayMetrics.DeserializeFromBand((ICargoReader) cargoCommandReader, this.ConnectedAdminBandConstants);
    }
    ...
    public void SetRunDisplayMetrics(CargoRunDisplayMetrics cargoRunDisplayMetrics)
    {
      ...
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (cargoRunDisplayMetrics));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (!cargoRunDisplayMetrics.IsValid(this.ConnectedAdminBandConstants))
      {
        ArgumentException argumentException = new ArgumentException(CommonSR.InvalidCargoRunDisplayMetrics);
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      int serializedByteCount = CargoRunDisplayMetrics.GetSerializedByteCount(this.ConnectedAdminBandConstants);
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoPersistedAppDataSetRunMetrics, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        cargoRunDisplayMetrics.SerializeToBand((ICargoWriter) cargoCommandWriter, this.ConnectedAdminBandConstants);
    }
    ...
    public CargoBikeDisplayMetrics GetBikeDisplayMetrics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedAppDataGetBikeMetrics, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        return CargoBikeDisplayMetrics.DeserializeFromBand((ICargoReader) cargoCommandReader, this.ConnectedAdminBandConstants);
    }
    ...
    public void SetBikeDisplayMetrics(CargoBikeDisplayMetrics cargoBikeDisplayMetrics)
    {
      ...
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (cargoBikeDisplayMetrics));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (!cargoBikeDisplayMetrics.IsValid(this.ConnectedAdminBandConstants))
      {
        ArgumentException argumentException = new ArgumentException(CommonSR.InvalidCargoBikeDisplayMetrics);
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      int serializedByteCount = CargoBikeDisplayMetrics.GetSerializedByteCount(this.ConnectedAdminBandConstants);
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoPersistedAppDataSetBikeMetrics, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        cargoBikeDisplayMetrics.SerializeToBand((ICargoWriter) cargoCommandWriter, this.ConnectedAdminBandConstants);
    }
    ...
    internal SleepNotification GetSleepNotification()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedAppDataGetSleepNotification, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        return SleepNotification.DeserializeFromBand((ICargoReader) cargoCommandReader);
    }
    ...
    internal void SetSleepNotification(SleepNotification notification)
    {
      ...
      int serializedByteCount = SleepNotification.GetSerializedByteCount();
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoPersistedAppDataSetSleepNotification, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
    ...
    internal LightExposureNotification GetLightExposureNotification()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedAppDataGetLightExposureNotification, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        return LightExposureNotification.DeserializeFromBand((ICargoReader) cargoCommandReader);
    }
    ...
    internal void SetLightExposureNotification(LightExposureNotification notification)
    {
      ...
      if (notification == null)
        throw new ArgumentNullException(nameof (notification));
      int serializedByteCount = LightExposureNotification.GetSerializedByteCount();
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoPersistedAppDataSetLightExposureNotification, serializedByteCount, CommandStatusHandling.ThrowOnlySeverityError))
        notification.SerializeToBand((ICargoWriter) cargoCommandWriter);
    }
    ...
    public CargoRunStatistics GetLastRunStatistics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedStatisticsRunGet, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        CargoRunStatistics cargoRunStatistics = CargoRunStatistics.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        return cargoRunStatistics;
      }
    }
    ...
    public CargoWorkoutStatistics GetLastWorkoutStatistics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedStatisticsWorkoutGet, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        CargoWorkoutStatistics workoutStatistics = CargoWorkoutStatistics.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
    ...
    public CargoSleepStatistics GetLastSleepStatistics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedStatisticsSleepGet, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        CargoSleepStatistics cargoSleepStatistics = CargoSleepStatistics.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        return cargoSleepStatistics;
      }
    }
    ...
    public CargoGuidedWorkoutStatistics GetLastGuidedWorkoutStatistics()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoPersistedStatisticsGuidedWorkoutGet, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        CargoGuidedWorkoutStatistics workoutStatistics = CargoGuidedWorkoutStatistics.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        return workoutStatistics;
      }
    }
      ...
      bool forceSync)
    {
      ...
      if (webTile1 != null)
      {
        ...
        try
        {
          flag = webTile1.HasRefreshIntervalElapsed(utcNow);
        }
        catch (Exception ex)
        {
          this.loggerProvider.LogException(ProviderLogLevel.Error, ex, "Error checking last sync time for Webtile Name: {0}, Id: {1}.", (object) webTile1.Name, (object) webTile1.TileId);
          flag = true;
        }
        if (forceSync | flag)
        {
...
label_7:
    ...
    private LogMetadataRange GetChunkRangeMetadata(int chunkCount)
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoLoggerGetChunkRangeMetadata, 4, serializedByteCount, writeArgBuf, CommandStatusHandling.DoNotCheck))
      {
        logMetadataRange = LogMetadataRange.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
      }
      if (logMetadataRange.EndingSeqNumber < logMetadataRange.StartingSeqNumber || (long) logMetadataRange.ByteCount > (long) (chunkCount * 4096))
      {
        Logger.Log(LogLevel.Warning, "The device returned an invalid metadata structure. RequestedChunkRangeSize = {0}. Returned ChunkRangeMetadata = (BytesCount = {1}, Start = {2}, End = {3}).", (object) chunkCount, (object) logMetadataRange.ByteCount, (object) logMetadataRange.StartingSeqNumber, (object) logMetadataRange.EndingSeqNumber);
    ...
    {
      int serializedByteCount = LogMetadataRange.GetSerializedByteCount();
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoLoggerDeleteChunkRange, serializedByteCount, CommandStatusHandling.DoNotCheck))
      {
        range.SerializeToBand((ICargoWriter) cargoCommandWriter);
        BandClient.CheckStatus(cargoCommandWriter.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
      }
    }
          ...
          using (MemoryPipeStream transferPipe = new MemoryPipeStream((int) rangeMetadata.ByteCount, progress, this.loggerProvider))
          {
            ...
            try
            {
              ...
              Action<ICargoWriter> writeArgBuf =
                closure_0 ?? (closure_0 = (Action<ICargoWriter>) (w => rangeMetadata.SerializeToBand(w)));

              using 
              (
                  CargoCommandReader cargoCommandReader = this.ProtocolBeginRead
                  (
                        DeviceCommands.CargoLoggerGetChunkRangeData, 
                        LogMetadataRange.GetSerializedByteCount(), 
                        (int) rangeMetadata.ByteCount, 
                        writeArgBuf, 
    ...
    private uint DeviceFileGetSize(ushort deviceCommand)
    {
      ...
      return (int) this.ProtocolRead(deviceCommand, 4, readData, statusHandling: CommandStatusHandling.DoNotThrow).Status != (int) DeviceStatusCodes.Success ? 0U : fileSize;
        ...
        {
            return Task.Run<string>((Func<string>)(() => this.GetProductSerialNumber()));
        }
    ...
    private Guid GetDeviceUniqueId()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoCoreModuleGetUniqueID, 66, CommandStatusHandling.DoNotThrow))
      {
        ...
        {
          BandException bandException = new BandException(CommonSR.InvalidGuidFromDevice, ex);
          Logger.LogException(LogLevel.Error, (Exception) bandException);
          throw bandException;
        }
      }
    ...
    {
        return Task.Run<bool>((Func<bool>)(() => this.GpsIsEnabled()));
    }
    ...
    private UserProfileHeader ProfileAppHeaderGet()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoProfileGetDataApp, 4, byteCount, writeArgBuf, CommandStatusHandling.DoNotCheck))
      {
        UserProfileHeader userProfileHeader = UserProfileHeader.DeserializeFromBand((ICargoReader) cargoCommandReader);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
    ...
    private byte[] ProfileGetFirmwareBytes()
    {
      ...
      using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoProfileGetDataFW, 4, byteCount, writeArgBuf, CommandStatusHandling.DoNotCheck))
      {
        ...
        byte[] numArray = cargoCommandReader.ReadExact(256);
        BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        return numArray;
      }
    }
    ...
    private void ProfileSetFirmwareBytes(UserProfile profile)
    {
      ...
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoProfileSetDataFW, UserProfile.GetFirmwareBytesSerializedByteCount(), CommandStatusHandling.DoNotCheck))
      {
        profile.SerializeFirmwareBytesToBand((ICargoWriter) cargoCommandWriter, this.ConnectedAdminBandConstants);
        BandClient.CheckStatus(cargoCommandWriter.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
      }
    }
    ...
    private void ProfileSetAppData(UserProfile profile)
    {
      ...
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoProfileSetDataApp, UserProfile.GetAppDataSerializedByteCount(this.ConnectedAdminBandConstants), CommandStatusHandling.DoNotCheck))
      {
        profile.SerializeAppDataToBand((ICargoWriter) cargoCommandWriter, this.ConnectedAdminBandConstants);
        BandClient.CheckStatus(cargoCommandWriter.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
      }
    }
    ...
    private void CheckIfUpdateValidForDevice(string firmwareVersion)
    {
      ...
      {
        BandException bandException = new BandException
                (
                    string.Format
                    (
                        CommonSR.ObsoleteFirmwareVersionOnDevice, 
                        (object) build, 
                        (object) num
                    )
                );
        Logger.LogException(LogLevel.Warning, (Exception) bandException);
        throw bandException;
      }
    }
    ...
    {
      string firmwareUpdateVersionFileRelativePath = Path.Combine("FirmwareUpdate", "FirmwareUpdate.json");
      string relativePath = Path.Combine("FirmwareUpdate", "FirmwareUpdate.bin");
      string str = Path.Combine("FirmwareUpdate", "FirmwareUpdateTemp.bin");
      ...
      else
      {
        ...
        {
          BandException bandException = new BandException(string.Format(CommonSR.FirmwareUpdateDownloadTempFileOpenError, (object) str), ex);
          Logger.LogException(LogLevel.Error, (Exception) bandException);
          throw bandException;
        }
        ...
        using (stream)
        {
          ...
          {
            e = (Exception) new BandCloudException(CommonSR.FirmwareUpdateDownloadError, ex);
          }
          if (e == null)
          {
            long num = long.Parse(updateInfo.SizeInBytes);
            if (stream.Length != num)
              e = (Exception) new BandException(string.Format(CommonSR.FirmwareUpdateDownloadTempFileSizeMismatchError, (object) stream.Length, (object) num));
          }
          if (e == null)
          {
            stream.Seek(0L, SeekOrigin.Begin);
            if (!this.AreEqual(this.platformProvider.ComputeHashMd5(stream), Convert.FromBase64String(updateInfo.HashMd5)))
              e = (Exception) new BandException(CommonSR.FirmwareUpdateIntegrityError);
          }
        }
        if (e != null)
        {
    ...
    private FirmwareUpdateInfo GetFirmwareUpdateVersionFromLocalFile(
      string firmwareUpdateVersionFileRelativePath)
    {
      FirmwareUpdateInfo firmwareUpdateInfo = (FirmwareUpdateInfo) null;
      ...
      FirmwareUpdateOverallProgress progressTracker)
    {
      ...
      cancellationToken.ThrowIfCancellationRequested();
      string str1 = Path.Combine("FirmwareUpdate", "FirmwareUpdate.bin");
      ...
      FirmwareUpdateOverallProgress progressTracker)
    {
      ...
      while (!this.DeviceTransport.IsConnected)
      {
        ...
        try
        {
          ...
          {
            this.DeviceTransport.Disconnect();
            BandException bandException = new BandException("Wrong device");
            Logger.LogException(LogLevel.Error, (Exception) bandException);
            throw bandException;
          }
        }
        catch
        {
          ...
          if (num > (byte) 40)
          {
            BandIOException bandIoException = new BandIOException(CommonSR.DeviceReconnectMaxAttemptsExceeded);
            Logger.LogException(LogLevel.Error, (Exception) bandIoException, "Exception occurred prior to firmware upload, but after BootIntoUpdateMode command");
      ...
      {
        BandException bandException = new BandException(CommonSR.DeviceNotInUpdateMode);
        Logger.LogException(LogLevel.Error, (Exception) bandException);
        throw bandException;
      }
      ...
      ProgressTrackerPrimitive progressTracker)
    {
      ...
      BandIOException bandIoException = new BandIOException(CommonSR.DeviceReconnectMaxAttemptsExceeded);
      Logger.LogException(LogLevel.Error, (Exception) bandIoException, "Exception occurred after firmware upload, when trying to check if device exited update mode");
      throw bandIoException;
    }
    ...
    private void ParseRemoteSubscriptionSample(CargoCommandReader reader)
    {
      ...
      while (reader.BytesRemaining > 0)
      {
        ...
        {
          Exception e = (Exception) new BandIOException(string.Format("An exception occurred reading subscription sample header #{0}", (object) num), ex);
        ...
        {
          Exception e = (Exception) new BandIOException(string.Format("An exception occurred parsing subscription data payload #{0}; Type: {1}, Size: {2}", (object) num, (object) sampleHeader.SubscriptionType, (object) sampleHeader.SampleSize), ex);
          Logger.LogException(LogLevel.Warning, e);
          throw e;
        }
        ++num;
      ...
      CancellationToken cancellationToken)
    {
      ...
      {
        string uploadId = string.Format("{0}-{1}", (object) this.storageProvider.GetFileCreationTimeUtc(relativeFilePath).ToString("yyyyMMddHHmmssfff"), (object) (int) fileIndex);
      ...
      CancellationToken cancellationToken)
    {
      ...
      switch (fileIndex)
      {
        ...
        case FileIndex.CrashDump:
          ...
          break;
        default:
          BandCloudException bandCloudException = new BandCloudException(CommonSR.UnsupportedFileTypeForCloudUpload);
          Logger.LogException(LogLevel.Error, (Exception) bandCloudException);
          throw bandCloudException;
      }
    ...
    private string[] GetAllResponses()
    {
      ...
      int bytesToRead = 322 * strArray.Length;
      try
      {
        using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoFireballUIGetAllSmsResponse, bytesToRead, CommandStatusHandling.DoNotCheck))
        {
          for (int index = 0; index < strArray.Length; ++index)
            strArray[index] = cargoCommandReader.ReadString(161);
          BandClient.CheckStatus(cargoCommandReader.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
        }
      }
      catch (BandIOException ex)
      ...
      NotificationFlags flagbits = NotificationFlags.UnmodifiedNotificationSettings)
    {
      ...
      if (name == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (name));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (body == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (body));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      ...
      IList<ITilePageElement> textFields)
    {
      ...
      if (textFields.Count == 0)
        throw new ArgumentException(string.Format(CommonSR.GenericCountZero, (object) nameof (textFields)), nameof (textFields));
    ...
    public void SendCalendarEvents(CalendarEvent[] events)
    {
      ...
      if (events == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (events));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if (events.Length > 8)
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.AppointmentsExceedLimit, (object) (ushort) 8));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
    ...
    private void GetDeviceSerialAndUniqueId(out string serial, out Guid uniqueId)
    {
      ...
      switch (this.ConnectedBandConstants.BandClass)
      {
        ...
        case BandClass.Envoy:
          ...
          break;
        default:
          this.loggerProvider.Log(ProviderLogLevel.Warning, string.Format("Unrecognized band class; PcbId = {0}", (object) this.FirmwareVersions.PcbId));
          uniqueId = Guid.Empty;
          break;
      }
    }
    ...
    private static Guid ConstructDeviceIdFromSerialNumber(string serialNumber)
    {
      if (serialNumber.Length == 12)
        return new Guid(string.Format("{0}{1}", (object) "FFFFFFFF-FFFF-FFFF-FFFF-", (object) serialNumber));
      return serialNumber.Length < 12 ? new Guid(string.Format("{0}{1}{2}", (object) "FFFFFFFF-FFFF-FFFF-FFFF-", (object) new string('0', 12 - serialNumber.Length), (object) serialNumber)) : new Guid(string.Format("{0}{1}", (object) "FFFFFFFF-FFFF-FFFF-FFFF-", (object) serialNumber.Substring(0, 12)));
    }
    ...
    private void SetTileThemesValidator(IDictionary<Guid, BandTheme> customThemes)
    {
      if (customThemes == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException("customColors");
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
    ...
    private void SetStartStripValidator(StartStrip tiles)
    {
      if (tiles == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (tiles));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      if ((long) tiles.Count > (long) this.GetTileCapacity() || (long) tiles.Count > (long) this.GetTileMaxAllocatedCapacity())
      {
        ArgumentException argumentException = new ArgumentException(string.Format(CommonSR.GenericCountMax, (object) nameof (tiles)));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
    }
    ...
    private void UpdateTileValidator(AdminBandTile tile)
    {
      if (tile == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (tile));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
    }
      ...
      bool alreadyOnDevice)
    {
      ...
      if (!(isDefault & alreadyOnDevice))
      {
        ...
        else
        {
          if (tile.Images == null)
          {
            ArgumentException argumentException = new ArgumentException(CommonSR.NewTileRequiresImages);
            Logger.LogException(LogLevel.Error, (Exception) argumentException);
            throw argumentException;
          }
    ...
    private void SetIconIndexValidator(uint iconIndex)
    {
      ...
      {
        ArgumentOutOfRangeException ofRangeException = new ArgumentOutOfRangeException(nameof (iconIndex));
        Logger.LogException(LogLevel.Error, (Exception) ofRangeException);
    ...
    private void InstalledAppListSet(IList<AdminBandTile> orderedList)
    {
      ...
      this.SetStartStripData((IEnumerable<TileData>) tileDataList, tileDataList.Count);
    }
    ...
    private AdminBandTile InstalledAppListGetTile(Guid guid)
    {
      ...
      {
        TileData data;
        using (CargoCommandReader cargoCommandReader = this.ProtocolBeginRead(DeviceCommands.CargoInstalledAppListGetTile, 16, bytesToRead, writeArgBuf, CommandStatusHandling.ThrowOnlySeverityError))
        {
          this.DeviceTransport.CargoStream.ReadTimeout = 60000;
          cargoCommandReader.ReadExact(buffer.Buffer, 0, buffer.Length);
          data = TileData.DeserializeFromBand((ICargoReader) cargoCommandReader);
    ...
    private void InstalledAppListSetTile(AdminBandTile tile)
    {
      ...
      using (CargoCommandWriter cargoCommandWriter = this.ProtocolBeginWrite(DeviceCommands.CargoInstalledAppListSetTile, TileData.GetSerializedByteCount(), CommandStatusHandling.DoNotCheck))
      {
        this.DeviceTransport.CargoStream.ReadTimeout = 60000;
        tileData.SerializeToBand((ICargoWriter) cargoCommandWriter);
        BandClient.CheckStatus(cargoCommandWriter.CommandStatus, CommandStatusHandling.ThrowOnlySeverityError, this.loggerProvider);
      }
    }
