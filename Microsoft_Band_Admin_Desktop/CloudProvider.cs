...
namespace Microsoft.Band.Admin
{
  internal sealed class CloudProvider
  {
    ...
    internal CloudProvider(ServiceInfo serviceInfo)
    {
      if (serviceInfo == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (serviceInfo));
      ...
      {
        ArgumentException argumentException = new ArgumentException(Microsoft.Band.Admin.Desktop.SR.ServiceAddressIsMissing, nameof (serviceInfo));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (string.IsNullOrWhiteSpace(serviceInfo.AccessToken))
      {
        ArgumentException argumentException = new ArgumentException(Microsoft.Band.Admin.Desktop.SR.AccessTokenIsMissing, nameof (serviceInfo));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (string.IsNullOrWhiteSpace(serviceInfo.DiscoveryServiceAddress))
      {
        ArgumentException argumentException = new ArgumentException(Microsoft.Band.Admin.Desktop.SR.DiscoveryServiceAddressIsMissing, nameof (serviceInfo));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      if (string.IsNullOrWhiteSpace(serviceInfo.DiscoveryServiceAccessToken))
      {
        ArgumentException argumentException = new ArgumentException(Microsoft.Band.Admin.Desktop.SR.DiscoveryServiceTokenIsMissing, nameof (serviceInfo));
        Logger.LogException(LogLevel.Error, (Exception) argumentException);
        throw argumentException;
      }
      ...
      this.authorizationHeader = new HttpHeader("Authorization", string.Format("WRAP access_token=\"{0}\"", (object) serviceInfo.AccessToken));
      this.authorizationHeaderForDiscoveryService = string.Format("WRAP access_token=\"{0}\"", (object) serviceInfo.DiscoveryServiceAccessToken);
      this.profileUri = new Uri(string.Format("{0}/v1/userprofiles", (object) serviceInfo.PodAddress));
      this.firmwareUpdateInfoUrlFormat = string.Format("{0}/api/FirmwareQuery?deviceFamily={{0}}&publishType=Latest&OneBL={{1}}&TwoUp={{2}}&currentFirmwareVersion={{3}}&IsForcedUpdate={{4}}", (object) serviceInfo.FileUpdateServiceAddress);
      this.ephemerisProfileUrl = string.Format("{0}/api/Ephemeris", (object) serviceInfo.FileUpdateServiceAddress);
      this.timezoneUpdateInfoUrl = string.Format("{0}/api/TimeZone", (object) serviceInfo.FileUpdateServiceAddress);
    }
      ...
      CancellationToken cancellationToken)
    {
      ...
      if (!stream.CanRead)
      {
        Exception e = (Exception) new ArgumentException(Microsoft.Band.Admin.Desktop.SR.CannotReadFromStream, nameof (stream));
        Logger.LogException(LogLevel.Error, e);
        throw e;
      }
      ...
      string requestUri = string.Format("{0}/v2/MultiDevice/UploadSensorPayload?logType={1}", (object) this.podAddress, (object) logType.ToCloudUploadPameterValue());
          ...
          using (HttpResponseMessage responseMessage = this.Send(httpClient, requestMessage, cancellationToken))
          {
            ...
            switch (responseMessage.StatusCode)
            {
              ...
              default:
                ...
                throw CloudProvider.CreateAppropriateException(responseMessage, responseContent, string.Format(CommonSR.FileUploadToCloudFailed, (object) logType));
            }
          }
        }
      }
      ...
      CancellationToken cancellationToken)
    {
      ...
      bool flag = true;
      stringBuilder.AppendFormat("{0}/v2/MultiDevice/GetUploadStatus?uploadIds=", (object) this.podAddress);
      foreach (string uploadId in uploadIDs)
      {
        stringBuilder.AppendFormat("{0}{1}", !flag ? (object) "," : (object) "", (object) uploadId);
        flag = false;
      }
        ...
        if (queryParams != null)
        {
          foreach (KeyValuePair<string, string> queryParam in queryParams)
          {
            if (string.Compare(queryParam.Key, "debug.force", StringComparison.OrdinalIgnoreCase) == 0)
            {
              ...
              stringBuilder1.AppendFormat("&{0}={1}", (object) queryParam.Key, (object) queryParam.Value);
          }
        }
      }
      ...
      using (HttpClient httpClient = this.CreateHttpClient(this.authorizationHeader))
      {
        ...
        using (HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Get, requestUri))
        {
          using (HttpResponseMessage responseMessage = this.Send(httpClient, requestMessage, cancellationToken))
          {
            ...
            this.LogRequestAndResponseMessages(LogLevel.Verbose, requestMessage, responseMessage, responseContent);
            using (Stream result = responseMessage.Content.ReadAsStreamAsync().Result)
              firmwareUpdateInfo = CargoClient.DeserializeJson<FirmwareUpdateInfo>(result);
          }
        }
      }
      if (firmwareUpdateInfo.IsFirmwareUpdateAvailable)
        Logger.Log(LogLevel.Info, "Firmware availability: {0} version from cloud: deviceFamily: {1}, Version: {2}", (object) firmwareUpdateInfo.IsFirmwareUpdateAvailable, (object) firmwareUpdateInfo.DeviceFamily, (object) firmwareUpdateInfo.FirmwareVersion);
      else
        Logger.Log(LogLevel.Info, "Firmware availability: {0}", (object) firmwareUpdateInfo.IsFirmwareUpdateAvailable);
      ...
      CancellationToken cancellationToken)
    {
      ...
      for (int index = 0; index < strArray.Length; ++index)
      {
        ...
        {
          ArgumentNullException argumentNullException = new ArgumentNullException(nameof (updateInfo));
          Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
          throw argumentNullException;
        }
      ...
      if (!flag)
      {
        BandCloudException bandCloudException = new BandCloudException(string.Format(CommonSR.FirmwareUpdateDownloadError, (object) stringBuilder));
      ...
      CancellationToken cancellationToken)
    {
      ...
      if (profile == null)
      {
        ArgumentNullException argumentNullException = new ArgumentNullException(nameof (profile));
        Logger.LogException(LogLevel.Error, (Exception) argumentNullException);
        throw argumentNullException;
      }
      ...
      {
        httpClient.Timeout = timeSpan;
        using (HttpRequestMessage requestMessage = new HttpRequestMessage(createNew ? HttpMethod.Post : HttpMethod.Put, string.Format("{0}/{1}", (object) this.profileUri, createNew ? (object) "post" : (object) "put")))
        {
      ...
      CancellationToken cancellationToken)
    {
      ...
      {
        httpClient.Timeout = timeSpan;
        using (HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Put, string.Format("{0}/put", (object) this.profileUri)))
        {
          requestMessage.Content = CloudProvider.CreateJsonContent(json);
          using (HttpResponseMessage responseMessage = this.Send(httpClient, requestMessage, cancellationToken))
          {
    ...
    {
      BandHttpException bandHttpException;
      switch (responseMessage.StatusCode)
      {
        case HttpStatusCode.Unauthorized:
        case HttpStatusCode.Forbidden:
          bandHttpException = (BandHttpException) new BandHttpSecurityException(responseContent, CloudProvider.FormatMessage(responseMessage, responseContent, message), innerException);
          break;
        default:
          bandHttpException = new BandHttpException(responseContent, CloudProvider.FormatMessage(responseMessage, responseContent, message), innerException);
          break;
      }
      return bandHttpException;
    }
