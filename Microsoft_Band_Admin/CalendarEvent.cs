...
namespace Microsoft.Band.Admin
{
  public sealed class CalendarEvent : NotificationBase
  {
    ...
    {
      int num1 = 0 + (2 + ProtoFileTime.GetSerializedProtobufByteCount()) + (1 + CodedOutputStream.ComputeUInt32Size((uint) (this.NotificationTime.HasValue ? this.NotificationTime : new ushort?(ushort.MaxValue)).Value));
      if (this.Duration != (ushort) 0)
        num1 += 1 + CodedOutputStream.ComputeUInt32Size((uint) this.Duration);
      int num2 = string.IsNullOrWhiteSpace(this.Location) ? num1 + (1 + CodedOutputStreamExtensions.ComputeStringSize(this.Title, 320)) : num1 + (1 + CodedOutputStreamExtensions.ComputeStringSize(this.Title, 40)) + (1 + CodedOutputStreamExtensions.ComputeStringSize(this.Location, 320));
      if (this.AcceptedState != CalendarEventAcceptedState.Accepted)
        num2 += 1 + CodedOutputStream.ComputeEnumSize((int) this.AcceptedState);
      CalendarEvent.ProtobufFlags protobufFlags = this.GetProtobufFlags();
      if (protobufFlags != CalendarEvent.ProtobufFlags.None)
        num2 += 1 + CodedOutputStream.ComputeUInt32Size((uint) protobufFlags);
      return num2;
    }
