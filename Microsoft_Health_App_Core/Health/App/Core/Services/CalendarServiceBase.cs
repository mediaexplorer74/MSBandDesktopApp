  ...
  public abstract class CalendarServiceBase : ICalendarService, ICalendarTileUpdateListener
  {
    ...
    public async void SaveLastSentCalendarEvents(IList<CalendarEvent> calendarEvents)
    {
      ...
      try
      {
        ...
        using (Stream fileStream = await (await (await this.fileSystemService.GetCalendarCacheFolderAsync().ConfigureAwait(false)).CreateFileAsync("CalendarEvents", CreationCollisionOption.ReplaceExisting).ConfigureAwait(false)).OpenAsync(FileAccess.ReadAndWrite).ConfigureAwait(false))
        {
          StreamWriter writer;
          int num;
          if (num != 4 && num != 5)
